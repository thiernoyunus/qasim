...
// MARK: - How the companion actually prays

extension PrayerName {
    /// Fard raka'at. The companion prays the obligatory units only, so the count
    /// on screen is countable and correct rather than an endless loop.
    var rakaat: Int {
        switch self {
        case .fajr: 2
        case .maghrib: 3
    ...
    /// One raka' on screen: stand, bow, prostrate, prostrate.
    /// ponytail: the two sujud reuse one sprite, so they read as a single long
    /// prostration. Add a sitting pose (jalsa) if that fidelity ever matters.
    static let posesPerRakaa = 4
    static let secondsPerPose: TimeInterval = 22

    /// How long the mat stays out: derived from the raka' count, not a flat timer.
    var prayerSeconds: TimeInterval {
        TimeInterval(rakaat * Self.posesPerRakaa) * Self.secondsPerPose
    }

    /// Pose for a given point in the prayer, driven by wall-clock elapsed time so
    /// relaunching mid-prayer resumes at the right place instead of restarting.
    func pose(elapsed: TimeInterval) -> QasimPose {
        let cycle: [QasimPose] = [.qiyam, .ruku, .sujud, .sujud]
        let total = rakaat * Self.posesPerRakaa
        guard total > 0 else { return .qiyam }
        let step = min(total - 1, max(0, Int(elapsed / Self.secondsPerPose)))
        return cycle[step % Self.posesPerRakaa]
    }

#if DEBUG
    /// Smallest check that fails if the raka' math drifts. Called once at launch.
    static func selfCheck() {
        assert(PrayerName.fajr.rakaat == 2 && PrayerName.maghrib.rakaat == 3)
        assert(PrayerName.dhuhr.rakaat == 4 && PrayerName.asr.rakaat == 4 && PrayerName.isha.rakaat == 4)
        assert(PrayerName.sunrise.rakaat == 0, "sunrise is not a prayer")
        for name in PrayerName.allCases where name.isSalah {
            // Every raka' must start standing and end prostrate, and the mat must
            // retire exactly when the last raka' finishes.
            var standings = 0
            let total = name.rakaat * posesPerRakaa
            for step in 0..<total {
                let pose = name.pose(elapsed: TimeInterval(step) * secondsPerPose + 1)
                if pose == .qiyam { standings += 1 }
            }
            assert(standings == name.rakaat, "\(name.title) drew \(standings) raka', expected \(name.rakaat)")
            assert(name.pose(elapsed: name.prayerSeconds - 1) == .sujud, "\(name.title) must end in sujud")
            assert(name.prayerSeconds > 0)
        }
    }
#endif
}
