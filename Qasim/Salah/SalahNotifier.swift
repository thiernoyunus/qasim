...
enum SalahNotifier {
    ...
    static func refresh(times: [PrayerTimes], leadMinutes: Int, enabled: Bool) async {
        ...
        let ours = pending.filter { $0.identifier.hasPrefix("qasim.salah.") }.map(\.identifier)
        ...
        for day in times {
            for name in PrayerTimes.salahOrder {
                ...
                await add(
                    id: "qasim.salah.now.\(name.rawValue).\(Int(date.timeIntervalSince1970))",
                ...
                await add(
                    id: "qasim.salah.soon.\(name.rawValue).\(Int(date.timeIntervalSince1970))",
